...
We will not give details of using R in this section: this will
all be covered by the worksheets. Here, we describe some of the outputs
we can get from R, and how we can use and interpret them.
...
Here, we will show the plots produced by R. You will find
...
We can add the straight line $y = x$ to the plot, and make
the axis labels
...
line, so the majority of students in the sample do gain weight 
over the course of the study. This appears to be the case irrespective
of the initial weight of the student.

If we are interested only in the gain in weight, we could also plot
a histogram of the difference between the final and initial weights:
...
We can use a box-and-whiskers plot to summarise continuous variables,
and to show how a continuous variable changes with a discrete
...
The line
in the middle of each box is the median service time for that group,
...
are shown as separate points. When no outlying plots are shown,
as in this case, the whiskers show the range of the data.

To demonstrate the effect of outliers,
suppose we there is another AM service time observed,
of 190 seconds. 

```{r, echo = FALSE}
fastfood_outlier <- fastfood %>%
    add_row(time = "AM", service = 190)

ggplot(data = fastfood_outlier) +
    geom_boxplot(aes(service, time)) +
    xlab("Service time (minutes)") +
    ylab("Time of day")
```

We can see on the box-and-whiskers plot that this new
point is an outlier, relative to the other AM service times.
