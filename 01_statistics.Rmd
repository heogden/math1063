...
kableExtra::kbl(head(wtgain, n = 10), col.names = c("Student number", "Initial weight (kg)", "Final weight (kg)"), booktabs = TRUE)
```
...
billionaires <- read_table("billionaires.txt") %>%
    mutate(region = recode(region, "M" = "Middle East",
                           ...
                           "E" = "Europe",
                           "O" = "Other"))
kableExtra::kbl(head(billionaires, n = 10), booktabs = TRUE)
```
...
```{r, echo = FALSE}
kableExtra::kbl(t(table(compfail)), booktabs = TRUE)
```
...
using colour of the lines to distinguish between the groups:
...
age as a scatter-plot, using the colour of the 
points to distinguish between different regions.


```{r, echo = FALSE, fig.width = 5, fig.height = 3, out.width = "60%", fig.align = "center"}
ggplot(data = billionaires) +
    geom_point(aes(x = age, y = wealth, colour = region)) +
    xlab("Age (years)") +
    ylab("Wealth (billions of dollars)")
```

The plot lets us immediatly read off some interesting
