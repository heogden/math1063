...
from $a$, \[\text{SSE}(a) = \sum_{i=1}^n (x_i - a)^2\]
or the sum of the absolute deviations from $a$, 
\[\text{SSA}(a) = \sum_{i=1}^n |x_i - a|.\]
...
minimised when $a$ is any value between $x_{\left(\frac{n}{2}\right)}$ and $x_{\left(\frac{n}{2} +1\right)}$. We defined the median to be the mean of these two values, so the median minimises the SSA.
:::
