...
As we have seen in the Chapter \@ref(intro-stats), there is uncertainty
...
probability. The Liverpool probability is the commentator's own subjective belief. The
...
Before we can state and use these axioms, we need to introduce some ideas some set theory (which
is taught in calculus). The sample space $S$ is called the whole set and it is composed 
of all possible
...
(2) From (1) and Axiom A1, $P\{0\}=1-P\{S\}=0$. Hence if $A$ and $B$ are mutually exclusive events, then $P\{A \cap B\}=0$.
...
To complete a specific task, supose one has to complete $k(\geq 1)$ sub-tasks sequentially.
If there are 
$n_i$
...
kableExtra::kbl(phones_data, booktabs = TRUE)
```
:::
