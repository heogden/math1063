...
:::{.example name="Dice throw continued"}
Return to the rolling of a fair die (Example \@ref(exm:dice)). Let
\begin{align*}
...
$nA$ outcomes in $A$ will make event $B$ occur; the number of such outcomes is given by the 
number of outcomes
...
<!-- TODO: consider replacing Figure 2.2. (or could just draw a "jigsaw puzzle" in lectures?) -->
...
**Proof**: 
Use the multiplication rule of conditional probability twice to give
\[P \{B_i |A\} =
...
The probability $P \{B_i |A\}$ is called the posterior probability of 
$B_i$ given $A$ and $P \{B_i \}$ is called the prior
probability.
...
The worked examples section contains further illustrations of Bayes' theorem.
<!-- TODO: will I include worked examples section? Delete reference if not -->
Note that
$\sum_{i=1}^k P \{B_i |A\} = 1$. Why? Nowadays Bayes' theorem is used to make statistical inference as
well. 
<!-- TODO: rephrase/delete this sentence? -->
...
<!-- TODO: add an extra step in deriving this from previous line -->

<!-- TODO: check for consistency of die vs dice -->
...
Note that independence is not the same as the mutually exclusive property.
When two events, $A$
and $B$, are mutually exclusive, the probability of their intersection,
$A \cap B$, is zero, i.e. $P \{A \cap B\} = 0$.
But if the two events are independent then $P \{A \cap B\} = P \{A\} \times P \{B\}$.
<!-- TODO: delete this para? -->
...
In recent years there have been some disastrous
...
<!-- TODO: put into theorem environment -->
**Independence of complementary events**: If $A$ and $B$ are independent, so are $A^\prime$
and $B^\prime$.
...
### Independence with more than three events
...
<!-- TODO: check for consistency in use of italics vs bold -->
The notion of independent events naturally leads to a set of independent trials
...
:::{.example name="Feller's road crossing example"}
The flow of traffic at a certain street crossing
...
Let $C_i$ denote the event that a car comes in the $i$th second and let $N_i$ denote the event that no
car arrives in the $i$th second.
<!-- TODO: note change in notation, $AB = A \cap B$? -->
                    
1. Consider $k = 0$. The pedestrian does not have to wait if and only if there are no cars in the
next three seconds, i.e. the event $N_1 N_2 N_3$. Now the arrival of the cars in successive seconds
are independent and the probability of no car coming in any second is $q = 1 - p$. Hence the
answer is $P \{N_1 N_2 N_3 \} = q \cdot q \cdot q = q^3$.
2. Consider $k = 1$. The person has to wait for one second if there is a car in the first second
and none in the next three, i.e. the event $C_1 N_2 N_3 N_4$. Hence the probability of that is $pq^3$.
...
second. Hence
\[P \{\text{wait 2 seconds}\} = P \{C_1 C_2 N_3 N_4 N_5 \} + P \{N_1 C_2 N_3 N_4 N_5 \} = p \cdot p \cdot q^3 + q \cdot p \cdot q^3 = pq^3.\]
4. Consider k = 3. The person has to wait for three seconds if and only if a car passes in the
3rd second but none in the next three, $C_3 N_4 N_5 N_6$. Anything can happen in the first two
seconds, i.e. $C_1 C_2$, $C_1 N_2$, $N_1 C_2$, $N_1 N_2$ --- all these four cases are mutually
exclusive. Hence
\begin{align*}
P \{\text{wait 3 seconds}\} &= P \{C_1 C_2 C_3 N_4 N_5 N_6 \} + P \{N_1 C_2 C_3 N_4 N_5 N_6 \} \\
& \; \; +P \{C_1 N_2 C_3 N_4 N_5 N_6 \} + P \{N_1 N_2 C_3 N_4 N_5 N_6 \} \\
&= p \cdot p \cdot p \cdot q^3 + p \cdot q \cdot p \cdot q^3 + q \cdot p \cdot p \cdot q^3 + q \cdot q \cdot p \cdot q^3 \\
&= pq^3.
\end{align*}
5. Consider $k = 4$. The person has to wait exactly $4$ seconds
...
<!-- TODO: consider whether to include old lecture 8 -->
