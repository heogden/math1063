...
It is easy to prove that $\int_{0}^{\infty} f(x) d x=1$. This is left as an exercise. To find the mean and variance of the distribution we need the gamma function as discussed in Section ??.
<!-- TODO: add reference -->

<!-- TODO: put into definition environment -->
...
will have to find a $q$ such that $F(q)=p$, where $p$ is the given probability. For example,
what value of $q$ will give us $F(q)=0.25$, so that $25 \%$ of the phones will have failed by
time $q$?

<!-- TODO: consider moving definition of quantiles outside of this specfic case -->
<!-- TODO: put into definition environment -->
For a given $0<p<1$, the $p$ th quantile (or $100 p$ percentile) of the random variable $X$ with $c d f(x)$ is defined to be the value $q$ for which $F(q)=p$.

The 50th percentile is called the median. The 25th and 75 th percentiles are called the quartiles.


<!-- TODO: check for earlier definition of pdf -->
:::{.example name="Uniform distribution"}
Consider the uniform distribution $U(a, b)$ in the interval $(a, b)$. Here $F(x)=\frac{x-a}{b-a}$. So for a given $p, F(q)=p$ implies $q=a+p(b-a)$.

For the uniform $U(a, b)$ distribution the median is $\frac{b+a}{2}$
...
\Rightarrow q &=-550 \times \log (1-p) .
\end{align*}
Review the rules of $\log$ in Section ???.
<!-- TODO: add reference or delete -->
Now we have
\[q=-550 \times \log (1-p) = \begin{cases}
\hline 158.2 & \text{for $p = 0.25$} \\
381.23 & \text{for $p = 0.50$} \\
762.46 & \text{for $p = 0.75$} \\
1266.422 & \text{for $p = 0.90$.}
  \end{cases}\]
...
qexp(p, rate)
```
calculates the $100 p$ percentile of the exponential
...
```{r, survival-probs, echo = FALSE, fig.cap = "$S(t)$ for $\\theta = 0.2, 0.5, 1$."}
...
<!-- TODO: note example just an application of the above? -->

:::{.example}
The time $T$ between any two successive arrivals in a hospital emergency department has probability density function:
\[f(t)= \begin{cases}
  \lambda e^{-\lambda t} & \text { if $t \geq 0$} \\
  0 & \text { otherwise.}\end{cases}
\]
Historically, on average the mean of these inter-arrival times is 5 minutes. 
Calculate 
...
<!-- TODO: motivate why normal distn important -->
<!-- TODO: split up into more subsections -->
...
It is easy to see that $f(x)>0$ for all $x$. We will now prove that

- **R1**: $\int_{-\infty}^{\infty} f(x) d x=1$ or total probability equals 1, so that $f(x)$ defines a valid pdf.
- **R2**: $E(X)=\mu$, i.e. the mean is $\mu$.
- **R3**: $\operatorname{Var}(X)=\sigma^2$, i.e. the variance is $\sigma^2$.

We denote the normal distribution by the notation $N\left(\mu, \sigma^2\right)$.
Suppose all of these hold (since they are proved below). Then we may remember 
the pdf of the normal distribution as:
<!-- TODO: remove "variable" notation? -->
...
\@ref(eq:std-normal-mean-zero) and hence **R2**.

To prove **R3**, i.e. $\operatorname{Var}(X)=\sigma^2$, we show that $\operatorname{Var}(Z)=1$ 
...
<!-- TODO: put in thoerem environment? -->
...
normal distribution, often denoted by $Z$, 
<!-- TODO: fix distribution noted as Z (vs random variable) -->
...
&\left.=\frac{1}{\sqrt{2 \pi}} \int_{\frac{a-\mu}{\sigma}}^\sigma \exp \left\{-\frac{z^2}{2}\right\} d z \quad \text { [substituted } z=\frac{x-\mu}{\sigma} \text { so that } d x=\sigma d z\right] \\
...
\[P(Z \leq z)=\Phi(z)=\int_{-\infty}^z \frac{1}{\sqrt{2 \pi}} \exp \left\{-\frac{u^2}{2}\right\} d u.\]
...
```
<!-- TODO: describe role of defaults before here -->
...
$\texttt{pnorm(40, mean=58, sd=32.25)} = 28.84\%$.
2. What percentage of students will get an A result (score greater than 70)? Answer: $\texttt{1 - pnorm(70, mean=58, sd=32.25)} = 35.49\%$.
3. What is the probability that a randomly selected student will score more than 90? Answer:
$\texttt{1 - pnorm(90, mean=58, sd=32.25)} = 0.1605$.
4. What is the probability that a randomly selected student will score less than 25? Answer:
$\texttt{pnorm(25, mean=58, sd=32.25) = 0.1531}$. Ouch!
...
$X \sim N (\mu_x = 42, \sigma_x^2 = 142 )$ and aim to define the scaling such that
$Y \sim N (\mu_y = 50, \sigma_y^2 = 152)$. 
...
$N(\mu+\sigma^2, \sigma^2)$ r.v. over its domain) }\\
...
<!-- TODO: write joint probability in table form for this example? -->
...
The joint pmf is
\[f(x, y) = \begin{cases}
  \frac{5}{36} & \text{if $x = -1, y = 1$} \\ 
 \frac{4}{36} & \text{if $x = -1, y = 2$} \\ 
 \frac{3}{36} & \text{if $x = -1, y = 3$} \\ 
 \frac{2}{36} & \text{if $x = -1, y = 4$} \\ 
 \frac{1}{36} & \text{if $x = -1, y = 5$} \\ 
 \frac{6}{36} & \text{if $x = 0, y = 0$} \\ 
 \frac{5}{36} & \text{if $x = 1, y = 1$} \\ 
 \frac{4}{36} & \text{if $x = 1, y = 2$} \\ 
 \frac{3}{36} & \text{if $x = 1, y = 3$} \\ 
 \frac{2}{36} & \text{if $x = 1, y = 4$} \\ 
...
where $a<x<b$ and $c<y<d$, then
\[\int_A f(x, y) d x d y=\int_c^d \int_a^b f(x, y) d x \, dy.\]
<!-- TODO: add more detail about how we use this when A is domain -->
Here $a$ and $b$ may depend upon $y$ but $c$ and $d$ should be free of $x$ and $y$. 
...
<!-- TODO: can we label as #joint-pdf-cts continued? -->
:::{.example}
...
and again more details will be provided in a second-year module. You will come across 
multivariate integrals in a second semester module. You will not be asked to do bivariate
integration in this module.
<!-- TODO: which modules? Rephrase to clarify what is needed for MATH1024 -->
...
<!-- TODO: give an example of this, or set as an exercise? -->
...
\end{align*}
using a result stated in Section ???. 
<!-- TODO: check reference to result -->
Thus, we have proved that the sum of independent binomial random variables with common probability is binomial as well. This is called the reproductive property of random variables. You are asked to prove this for the Poisson distribution in an exercise sheet.
<!-- TODO: check exercise sheet contains this question, or remove comment -->
:::
...
```{r, fig.cap = "Distribution of normalised sample means for samples of different sizes. Initially very skew (original distribution, $n=1$ ) becoming rapidly closer to standard normal (dashed line) with increasing $n$."}
f <- function(z, n) {
...
<!-- TODO: generate plot? Really histograms? But then what bin width? -->
