...
\[E(X)= \begin{cases}\sum_{x} x f(x) & \text { if } X \text { is discrete } \\ \int_{-\infty}^{\infty} x f(x) d x & \text { if } X \text { is continuous }\end{cases}\]
...
::: {.proof}
The proof is give for the continuous case. In the discrete case replace integral $\left(\int\right)$ by summation $\left(\sum\right)$.
\begin{align*}
...
The density (pdf) is much easier to remember and work with when the mean $\mu=0$ and variance $\sigma^2=1$. In this case, we simply write:
...
where $A = \{(x, y) \text{ such that } a<x<b \text{ and } c<y<d\
$. Then
...
```{r, fig.cap = "Distribution of normalised sample means for samples of different sizes. Initially very skew (original distribution, $n=1$ ) becoming rapidly closer to standard normal (dashed line) with increasing $n$.", fig.width = 5, fig.height = 3, out.width = "60%", fig.align = "center"}
f <- function(z, n) {
